...
type EditBtnProps = {
  ...
  schema: JSONSchema, featureTypeName: string, onChange: () => any
};
...
export class EditMapFeatureButton extends React.Component<EditBtnProps, EditBtnState> {
  ...
  render() {
    ...
    // @ts-ignore
    const fields = featureTypeName == 'Entrance' ? mapFeature.entrance_fields : mapFeature;
    return <button className={popupBtn} onClick={() => this.setState({editing: true})}>
      <WWIcon icon="edit_note" outline/> Lisätiedot
      {editing && schema && ReactDOM.createPortal(<Modal title="Lisätiedot" onClose={this.onCancel}>
        <div className="p-2">
          <MapFeatureForm schema={schema} onSubmit={this.onSubmit} onCancel={this.onCancel}
                          // @ts-ignore
                          featureTypeName={featureTypeName} mapFeature={fields}/>
        </div>
      </Modal>, document.body)}
    </button>;
  }
  ...
  onSubmit = ({formData}: any) => {
    if (this.props.featureTypeName == 'Entrance')
      ...
      Object.assign(this.props.mapFeature, formData);
    this.onCancel();
    this.props.onChange();
  }
}