...
export default class UnloadingPlaceAccessPoints extends React.Component<UnloadingPlaceAccessPointsProps, UnloadingPlaceAccessPointsState> {
  ...
  render() {
    ...
    return <div className="mt-1">
      ...
      <Button size="sm" color="primary" outline className="btn-compact"
              onClick={() => this.setState({open: true})}>Edit access points</Button>
      {open &&
        <Modal onClose={() => this.setState({open: false})}
          ...
          <div style={{height: 400}}>
            <Map latLng={latLng} extraLayers={[this.getMapLayer()]} zoom={19} onClick={this.addPoint}/>
          </div>
          <div className="p-2">
  ...
  getMapLayer() {
    ...
    unloadingPlace.access_points.forEach((accessPoint: Location, index: number) => {
      const latLng = {lng: accessPoint.lon, lat: accessPoint.lat} as LatLngLiteral;
      const marker = L.circleMarker(latLng, markerStyle);
      marker.on('click', (e: any) => {this.removePoint(index);});
