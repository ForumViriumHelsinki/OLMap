...
export default class OSMEntranceCreationModal extends React.Component<OSMEntranceCreationModalProps, OSMEntranceCreationModalState> {
  state = initialState;
  ...
  render() {
    ...
    // @ts-ignore
    const [lon, lat] = entranceCreator ? entranceCreator.entrancePoint.geometry.coordinates: [];
    const {accessPoint} = entranceCreator || {};
    const accessLatLng = accessPoint && [...accessPoint.geometry.coordinates].reverse() as LatLngExpression;
    ...
    return <Modal onClose={onClose} title="Create OSM entrance"><ModalBody>
      {error && <ErrorAlert message={error} status/>}
      {!entranceCreator ? 'Loading...' : <>
        <Map latLng={[lat, lon]} height="50vh" zoom={21}>
          ...
          {accessLatLng && createWay && <>
            <CircleMarker center={accessLatLng} radius={4} fill fillOpacity={1} color={green} />
            <Polyline positions={[[lat, lon], accessLatLng]} color={green} />
          </>}
        </Map>
        <div className="mt-2">
          <button className="btn btn-outline-primary btn-sm" onClick={() => this.setState({createWay: !createWay})}>
  ...
  createOSMEntrance = () => {
    ...
    entranceCreator.createEntrance(osmContext, entrance.as_osm_tags, createWay && this.getEntrancePathTags())
    .then(onCreated)
