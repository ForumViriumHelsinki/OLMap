...
export default class MapFeatureOSMLink extends React.Component<
  MapFeatureOSMLinkProps,
  MapFeatureOSMLinkState
> {
  ...
  render() {
    const { osmImageNote, mapFeature, osmFeature, featureTypeName } =
      this.props;
    ...
    return (
      <>
          ...
          <table className="table table-bordered table-sm mt-2 mb-2">
            <tbody>
              <tr>
                <th colSpan={3}>
                  OSM:{" "}
                  {mapFeature.osm_feature
                    ? mapFeature.osm_feature + " (not found)"
                    : "Not linked"}
                  <button
                  ...
                  {canAdd && (
                    <>
                      <button
                        className="btn btn-compact btn-sm btn-outline-primary mr-1"
                        onClick={this.addToOSM}
                      >
                        Add to OSM
                      </button>
                      <button
                        className="btn btn-compact btn-sm btn-outline-secondary mr-1"
          ...
          <>
              ...
              <tbody>
                <tr>
                  <th colSpan={3}>
                    OSM:{" "}
                    <a
                      href={`https://www.openstreetmap.org/${osmFeature.type}/${osmFeature.id}`}
                      target="osm"
                    >
                    ...
                    <button
                      className="btn btn-light btn-compact btn-sm ml-1"
                      onClick={this.unlinkOsmFeature}
                    >
                      <Icon icon="link_off" />
                    </button>
                  </th>
            ...
            {discrepantTags && discrepantTags.length > 0 && (
              <div className="mb-4">
                ...
                <button
                  className="btn btn-compact btn-sm btn-outline-danger mr-1"
                  onClick={this.replaceOSM}
                >
                  Replace OSM
                </button>
                <button
        ...
        {showChangeset && (
          <OpenOSMChangesetModal
            onClose={() => this.setState({ showChangeset: false })}
            onCreated={osmFeature ? this.updateOSMNode : this.createOSMNode}
          />
        )}
        {showOSMEntranceModal && (
          <OSMEntranceCreationModal
            osmImageNote={osmImageNote}
            osmContext={osmEditContext}
            entrance={mapFeature}
  ...
  linkOSMFeature() {
    const { osmImageNote, mapFeature, featureTypeName, nearbyFeatures } =
      this.props;
    let osmFeature;
    ...
    if (featureTypeName == "Workplace" && mapFeature.name) {
      ...
      if (!osmFeature)
        osmFeature = nearbyFeatures.find(
          (f) =>
            f.tags.name &&
            f.tags.name.search(new RegExp(mapFeature.name, "i")) > -1,
        );
    }
    ...
    if (osmFeature) {
      ...
      if (!osmImageNote.osm_features.includes(osmId))
  ...
  addToOSM = () => {
    ...
    this.tagsToSave = osmFeature
      ? { ...osmFeature.tags, ...mapFeature.as_osm_tags }
      : { ...mapFeature.as_osm_tags };
    this.saveToOSM();
  };
  ...
  updateOSMNode = (osmContext: OSMEditContextType) => {
    ...
    const props = {
      changesetId: osmContext.changeset.id,
      node: osmFeature,
      tags: this.tagsToSave,
    };
    osmApiCall(`node/${osmFeature.id}`, UpdateNode, props, osmContext).then(
      ({ response, text }) => {
  ...
  createOSMNode = (osmContext: OSMEditContextType) => {
    ...
    if (featureTypeName == "Entrance")
      return this.setState({ showOSMEntranceModal: true });
  ...
  onOSMNodeCreated = (osmNode: OSMFeature) => {
    const { osmImageNote, addNearbyFeature, mapFeature, saveFeature } =
      this.props;
