...
class TypeLabel extends React.Component<{type: WorkplaceType}> {
  ...
  render() {
    ...
    return <>
      <img style={{height: 16, width: 0}} src={this.iconUrl(type)}
           onError={e => (e.target as Element).remove()}
           onLoad={e => (e.target as HTMLElement).style.width = '16px'}/> {type.label}
    </>;
  }
}
  ...
  render() {
    ...
    return <div key={type.id}>
      <span className="list-group-item list-group-item-action"
        ...
        <span className="clickable" onClick={() => onSelect(type)}><TypeLabel type={type}/></span>
      </span>
      {(expanded || expandAll) && type.children.map(t =>
...
export default class WorkplaceTypeWidget extends React.Component<WorkplaceTypeWidgetProps, WorkplaceTypeWidgetState> {
  state = initialState;
    ...
    return <div className="form-control">
      ...
      {expanded &&
        <Modal onClose={() => this.setState({expanded: false})} title="Select workplace type">
          <input className="form-control" placeholder="Filter..." value={filter} style={{width: '100%'}}
                 onChange={e => this.setState({filter: e.target.value})}/>
          <div className="list-group">
            {this.getFilteredTypes().map(t =>
              <TypeItem type={t} level={1} expandAll={(filter || '').length > 2}
                        onSelect={t => {this.setState({expanded: false}); onChange(t.id)}}/>)}
          </div>
        </Modal>
      }
    </div>;
  ...
  private fetchWorkplaceTypes() {
    sessionRequest(workplaceTypesUrl).then(response => response.json()).then((types: WorkplaceType[]) => {
      ...
      types.forEach(t => t.parents.forEach(parentId => (typesIndex[parentId].children as WorkplaceType[]).push(t)));
  ...
  private getFilteredTypes() {
    ...
    Object.values(typesIndex).filter(t => [t.label].concat(t.synonyms).join(' ').search(regexp) > -1).forEach(recordMatch);

    const filterTypes = (types: WorkplaceType[]) =>
      types.filter(t => filterIndex[t.id]).map(t => {
        const filteredT = {...t};
