...
export default class OSMImageNoteRelatedPlaces extends React.Component<OSMImageNoteRelatedPlacesProps, OSMImageNoteRelatedPlacesState> {
  state = initialState;
  ...
  render() {
    ...
    return <>
      <div className="list-group-item">
        ...
        {!editing && (
          !osm_features.length ? '-' :
            !nearbyPlaces.length ? 'Loading...' : osm_features.map(featureId => {
              const feature = nearbyPlaces.find((a) => a.id == featureId);
              return feature && <span key={featureId}>
                {featureId != osm_features[0] && ', '}{osmFeatureLabel(feature)}
              </span>
        }))}
      </div>
