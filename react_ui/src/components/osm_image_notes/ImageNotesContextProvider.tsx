...
import {AppContext, ImageNotesContextType, MapFeatureTypes, OSMImageNote, ImageNotesContext} from "components/types";
...
export default class ImageNotesContextProvider extends React.Component<ImageNotesContextProps, ImageNotesContextState> {
  ...
  render() {
    ...
    const context: ImageNotesContextType = {
      ...this.state, user, addNote: this.addNote, refreshNote: this.refreshNote, loadImageNotes: this.loadImageNotes};
    return <ImageNotesContext.Provider value={context}>
      {children}
