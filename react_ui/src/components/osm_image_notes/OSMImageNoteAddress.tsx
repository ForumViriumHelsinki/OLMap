...
export default class OSMImageNoteAddress extends React.Component<OSMImageNoteAddressProps, OSMImageNoteAddressState> {
  state = initialState;
  ...
  render() {
    ...
    return <>
      <div className="list-group-item">
        ...
        {!editing && (
          !addresses.length ? '-' :
            !nearbyAddresses.length ? 'Loading...' : addresses.map(addressId => {
              const address = nearbyAddresses.find((a) => a.id == addressId);
              return address && <span key={addressId}>
                {addressId != addresses[0] && ', '}{osmAddressString(address.tags)}
              </span>
        }))}
      </div>
      {editing &&
        <OSMFeatureList location={locationTuple} selectedFeatureIds={addresses} showFilters={false}
                        onChange={saveAddresses} OSMFeatures={nearbyAddresses} />}
    </>;
  }
}
